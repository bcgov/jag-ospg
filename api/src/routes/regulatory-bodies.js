...
async function getById(req, res) {
	...
	const regulatoryBody = await models.regulatoryBody.findByPk(id);
	if (regulatoryBody) {
		res.status(200).json(regulatoryBody);
	} else {
...
async function create(req, res) {
	if (req.body.id) {
		...
		await models.regulatoryBody.create(req.body);
		res.status(201).end();
	}
};
...
async function update(req, res) {
	...
	if (req.body.id === id) {
		await models.regulatoryBody.update(req.body, {
			where: {
				id: id
			}
		});
...
async function remove(req, res) {
	const id = getIdParam(req);
	await models.regulatoryBody.destroy({
		where: {
			id: id
		}
	});
